...
uint8_t magic[] = {0x52, 0x42, 0x31, 0x2D, 0x34, 0x2D, 0x36, 0x34, 0x00};
...
void encrypt(uint64_t *b, uint64_t *s, unsigned r)
{
 int i, end;
 ...
 b[2] += s[0];
 for(i = 1; i <= end; i++)
  b[i & 3] = rot_l(b[i & 3] ^ b[(i + 1) & 3], b[(i + 1) & 3]) + s[i];
 b[1] += s[i];

 return;
}

void decrypt(uint64_t *b, uint64_t *s, unsigned r)
{
 int i, end;
 ...
 b[1] -= s[end + 1];
 for(i = end; i > 0; i--)
  b[i & 3] = rot_r(b[i & 3] - s[i], b[(i + 1) & 3]) ^ b[(i + 1) & 3];
 b[2] -= s[0];
...
void write_oef(FILE *inf, FILE *outf, uint64_t *ds, uint64_t *ns, unsigned r)
{
 int c, i;
 uint8_t seed[] = {0x6E, 0x6F, 0x6E, 0x63, 0x65, 0x31};
 uint64_t n[4], b[4], st[4];
...
void read_oef(FILE *inf, FILE *outf, uint64_t *ds)
{
 int i, c, end = 0;
 unsigned r = 0;
 uint64_t n[4], b[4], st[4];
...
uint64_t *key_sched(uint8_t *kb, unsigned kbl, unsigned r)
{
 ...
 /* initialize schedule */
 s[0] = 0x62724AA780976032;
 for(ind = 1; ind < sl; ind++)
  s[ind] = s[ind - 1] + 0xA0855A7B728F38CB;
