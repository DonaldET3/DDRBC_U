...
uint8_t magic[] = {0x52, 0x42, 0x31, 0x2D, 0x33, 0x32, 0x00};
...
void encrypt(uint32_t *b, uint32_t *s, unsigned r, unsigned wpb)
{
 int i, end;
 unsigned m;
...
void decrypt(uint32_t *b, uint32_t *s, unsigned r, unsigned wpb)
{
 int i, end;
 unsigned m;
...
void write_oef(FILE *inf, FILE *outf, uint32_t *ds, uint32_t *ns, unsigned r, unsigned wpb)
{
 int c, i;
 unsigned opb;
 ...
 if((n = calloc(wpb, 4)) == NULL) fail();
 if((b = calloc(wpb, 4)) == NULL) fail();
 if((st = calloc(wpb, 4)) == NULL) fail();
 ...
 for(i = 3; i >= 0; i--)
  if(putc(0xFF & (((uint32_t)1) >> (i * 8)), outf) == EOF)
  {
 ...
 for(i = 3; i >= 0; i--)
  if(putc(0xFF & (((uint32_t)r) >> (i * 8)), outf) == EOF)
  {
   perror("write rounds");
   exit(EXIT_FAILURE);
  }
 ...
 for(i = 3; i >= 0; i--)
  if(putc(0xFF & (((uint32_t)wpb) >> (i * 8)), outf) == EOF)
  {
   perror("write rounds");
   exit(EXIT_FAILURE);
  }
 ...
 while(true)
 {
  ...
  for(i = 0; i < opb; i++)
  {
   if((c = getc(inf)) == EOF)
    break;
   b[i / 4] |= ((uint32_t)c) << ((3 - (i & 3)) * 8);
  }

  /* mark end */
  if(i < opb)
   b[i / 4] |= ((uint32_t)0x80) << ((3 - (i & 3)) * 8);
...
void read_oeh(FILE *inf, struct cipher_settings *cs)
{
 ...
 for(i = 3; i >= 0; i--)
 {
  ...
  cs->wpb |= ((uint32_t)c) << (i * 8);
 }

 opb = cs->wpb * 4;

 if((cs->n = calloc(cs->wpb, 4)) == NULL) fail();
...
uint32_t *key_sched(uint8_t *kb, unsigned kbl, unsigned r, unsigned wpb)
{
