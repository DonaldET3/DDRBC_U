...
uint8_t magic[] = {0x52, 0x42, 0x31, 0x2D, 0x34, 0x2D, 0x31, 0x36, 0x00};
...
void encrypt(uint16_t *b, uint16_t *s, unsigned r)
{
 int i, end;
...
void decrypt(uint16_t *b, uint16_t *s, unsigned r)
{
 int i, end;
...
void write_oef(FILE *inf, FILE *outf, uint16_t *ds, uint16_t *ns, unsigned r)
{
 int c, i;
 uint8_t seed[] = {0x6E, 0x6F, 0x6E, 0x63, 0x65, 0x31};
 uint16_t n[4], b[4], st[4];
...
void read_oef(FILE *inf, FILE *outf, uint16_t *ds)
{
 int i, c, end = 0;
 unsigned r = 0;
 uint16_t n[4], b[4], st[4];
...
uint16_t *key_sched(uint8_t *kb, unsigned kbl, unsigned r)
{
 unsigned ind, kwl, sl, end;
 uint16_t *kw, *s, a, b, i, j;

 /* find key word length */
 kwl = (kbl / 2) + ((kbl & 1) != 0) + (kbl == 0);
 ...
 for(ind = 0; ind < kbl; ind++)
  kw[ind / 2] |= (((uint16_t)kb[ind]) << ((1 - (ind & 1)) * 8));
