...
uint8_t magic[] = {0x52, 0x42, 0x31, 0x2D, 0x36, 0x34, 0x00};
...
void encrypt(uint64_t *b, uint64_t *s, unsigned r, unsigned wpb)
{
 int i, end;
 unsigned m;
...
void decrypt(uint64_t *b, uint64_t *s, unsigned r, unsigned wpb)
{
 int i, end;
 unsigned m;
...
void write_oef(FILE *inf, FILE *outf, uint64_t *ds, uint64_t *ns, unsigned r, unsigned wpb)
{
 int c, i;
 unsigned opb;
 ...
 if((n = calloc(wpb, 8)) == NULL) fail();
 if((b = calloc(wpb, 8)) == NULL) fail();
 if((st = calloc(wpb, 8)) == NULL) fail();
 ...
 for(i = 7; i >= 0; i--)
  if(putc(0xFF & (((uint64_t)1) >> (i * 8)), outf) == EOF)
  {
 ...
 for(i = 7; i >= 0; i--)
  if(putc(0xFF & (((uint64_t)r) >> (i * 8)), outf) == EOF)
  {
   perror("write rounds");
   exit(EXIT_FAILURE);
  }
 ...
 for(i = 7; i >= 0; i--)
  if(putc(0xFF & (((uint64_t)wpb) >> (i * 8)), outf) == EOF)
  {
   perror("write rounds");
   exit(EXIT_FAILURE);
  }
 ...
 for(i = 0; i < 6; i++)
  seed[i % opb] += rseed[i];
 for(i = 0; i < opb; i++)
  n[i / 8] |= ((uint64_t)seed[i]) << ((7 - (i & 7)) * 8);
 /* generate nonce */
 encrypt(n, ns, r, wpb);

 /* write nonce */
 for(i = 0; i < opb; i++)
  if(putc(0xFF & (n[i / 8] >> ((7 - (i & 7)) * 8)), outf) == EOF)
  {
   perror("write nonce");
   exit(EXIT_FAILURE);
  }
...
void read_oeh(FILE *inf, struct cipher_settings *cs)
{
 ...
 for(i = 7; i >= 0; i--)
 {
  ...
  cs->wpb |= ((uint64_t)c) << (i * 8);
 }

 opb = cs->wpb * 8;

 if((cs->n = calloc(cs->wpb, 8)) == NULL) fail();
...
uint64_t *key_sched(uint8_t *kb, unsigned kbl, unsigned r, unsigned wpb)
{
 unsigned ind, kwl, sl, end;
 uint64_t *kw, *s, a, b, i, j;

 /* find key word length */
 kwl = (kbl / 8) + ((kbl & 7) != 0) + (kbl == 0);
 if((kw = calloc(kwl, 8)) == NULL)
 {
 ...
 for(ind = 0; ind < kbl; ind++)
  kw[ind / 8] |= (((uint64_t)kb[ind]) << ((7 - (ind & 7)) * 8));

 /* schedule length */
 sl = (wpb * r) + 2;
 if((s = malloc(sl * 8)) == NULL)
 {
 ...
 /* initialize schedule */
 s[0] = 0xAAAAAAAAAAAAAAAA;
 for(ind = 1; ind < sl; ind++)
  s[ind] = s[ind - 1] + 0x0123456789ABCEDF;
