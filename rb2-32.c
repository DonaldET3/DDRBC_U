...
/* RB2-32 */
uint8_t magic[] = {0x52, 0x42, 0x32, 0x2D, 0x33, 0x32, 0x00};
...
/* read word byte */
int rwb(uint32_t w, int i)
{
 return (w >> ((3 - i) * 8)) & 0xFF;
}

/* write word byte */
uint32_t wwb(uint32_t w, int i, int b)
{
 w &= (((uint32_t)0) - 1) ^ ((uint32_t)0xFF) << ((3 - i) * 8);
 return w | ((((uint32_t)b) & 0xFF) << ((3 - i) * 8));
}
...
void encrypt(uint32_t *b, struct options *opts)
{
 int i, end;
 unsigned m;
...
void decrypt(uint32_t *b, struct options *opts)
{
 int i, end;
 unsigned m;
...
void hash_key(uint32_t *b, unsigned hbwn, struct options *opts)
{
 int i, end;
 unsigned m;
...
void get_text(uint32_t *b, unsigned hbwn)
{
 int i = 0, c, hbbn;
 ...
 while((c != EOF) && (c != '\0') && (c != '\n'))
 {
  b[i / 4] = wwb(b[i / 4], i % 4, c + rwb(b[i / 4], i % 4));
  i = (i + 1) % hbbn;
  c = getchar();
 }
...
void get_hex(uint32_t *b, unsigned hbwn)
{
 int i = 0, c, hbbn;
...
void write_header(FILE *outf, struct options *opts)
{
 ...
 write_word((uint32_t)opts->r, outf);

 /* write number of words per block */
 write_word((uint32_t)opts->wpb, outf);
...
void read_header(FILE *inf, struct options *opts)
{
 ...
 /* read nonce */
 opts->n = calloc((size_t)opts->wpb, 4);
 for(i = 0; i < opts->wpb; i++)
...
void e_file(struct options *opts, char *inn, char *outn)
{
 ...
 khb = calloc((size_t)hbwn, 4);
 nhb = calloc((size_t)hbwn, 4);
 opts->n = calloc((size_t)opts->wpb, 4);
